...
// older release that predates PostToolUse and the subagent hooks. Their
// hooks.json has the three legacy events but the CLI installs six today.
// Detection must surface every gap so doctor can prompt `entire enable`.
